...
"#, temp_path.display());
    ...
    let results = search_dir(temp_path, &config, rule, true).unwrap();
...
"#, temp_path.display(), temp_path.display());
    ...
    let rust_results = search_dir(temp_path, &config, &config.rules[0], true).unwrap();
    let yml_results = search_dir(temp_path, &config, &config.rules[1], true).unwrap();
...
"#, nonexistent_path.display());
    ...
    let results = search_dir(nonexistent_path, &config, rule, true).unwrap();
